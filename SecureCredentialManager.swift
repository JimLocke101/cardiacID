...
//  IMPORTANT: Add this file to BOTH iOS and watchOS targets, but only ONCE per target
//
...
// MARK: - Main Credential Manager Class
...
final class SecureCredentialManager {
    ...
    private init() {
        #if os(watchOS)
        // Initialize encryption for watchOS
        if let keyData = UserDefaults.standard.data(forKey: "encryption_key_v2") {
            encryptionKey = SymmetricKey(data: keyData)
        } else {
            encryptionKey = SymmetricKey(size: .bits256)
            UserDefaults.standard.set(encryptionKey.withUnsafeBytes { Data($0) }, forKey: "encryption_key_v2")
        }
        #endif
    }
}
...
extension SecureCredentialManager {
    ...
    private func deleteFromKeychain(forKey key: CredentialKey) throws {
        ...
        guard status == errSecSuccess || status == errSecItemNotFound else {
            throw CredentialError.storageError("Keychain delete failed: \(status)")
        }
    }
